import Day12 as D12 hiding (main)
import Test.HUnit
        ...
        TestLabel "run" $
          TestList
            [ TestCase $ assertEqual "cpy" (D12.Runtime (M.fromList [(D12.A, 8)]) [D12.Cpy (D12.Value 8) D12.A] 1) (execState D12.step (D12.Runtime M.empty [D12.Cpy (D12.Value 8) D12.A] 0)),
              TestCase $ do x <- readFile "test/day12test.txt"; assertEqual "test" D12.Runtime {D12.mem_ = M.fromList [(D12.A, 42)], D12.prog_ = [D12.Cpy (D12.Value 41) D12.A, D12.Inc D12.A, D12.Inc D12.A, D12.Dec D12.A, D12.Jnz (D12.Ref D12.A) 2, D12.Dec D12.A], D12.ptr_ = 6} (execState D12.runProg (D12.Runtime M.empty (D12.parseProg x) 0))
            ]
      ]

main :: IO ()
main = runTestTT testDay12 >>= print