...
rect args (Screen b) =
 let (rows,cols) = parseRectArgs args in
  Screen $ zipWith  (\ i bs -> if i < rows then drawRow cols bs else bs ) [0..] b
...
rotateR args (Screen b) =
 let (rowNo,n) = parseRotateArgs args in
  Screen $ zipWith (\ i bs -> if i == rowNo then rotate n bs else bs ) [0..] b
...
main =
 readFile "inputs/day08.txt" >>= \inString ->
  ...
  putStrLn (taskA inString) >> -- 166
  putStrLn "Answer to B is shown together with A above! :)"